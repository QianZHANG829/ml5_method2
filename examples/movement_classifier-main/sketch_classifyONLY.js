...
function draw() {
  ...
  text(modelDetails.model, width / 16, height / 8);
}
...
function startDetection() {
  ...
  setTimeout(() => {
    console.log("Detection started for 20 seconds.");
    // Clear previous results
    poseLabel = "";
    sequence = [];
    detecting = true;
    // Stop detection after 5 seconds if not already stopped
    setTimeout(() => {
      if (detecting) {
        detecting = false;
        console.log("Detection period ended.");
        // If sequence is not empty, force prediction with what has been collected:
        if (sequence.length > 0) {
          while(sequence.length < CAPTURE_FRAMES) {
            sequence.push(sequence[sequence.length-1]);//javascript time preceision is not reliable
          }
          classifier.predict(sequence, gotResults);
          sequence = [];
        } else {
    ...
    }, (CAPTURE_FRAMES / FPS) * 1000); // 这里为 (600 / 30) * 1000 = 20000 毫秒
  }, 2000);
}
