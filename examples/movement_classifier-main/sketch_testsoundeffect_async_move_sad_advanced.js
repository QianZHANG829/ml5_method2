...
function initLayer2(){
  ...
  /* ---------- 3. Shift 节点 ---------- */
  const violinShift = new Tone.PitchShift(0)          // 已有的 vVerb/vDelay 仍然接在后面
        .connect(splitter);
        
  violinShift = new Tone.PitchShift(0)        // 只赋值一次（全局变量）
  ...
  const fallFiles = {
    ...
    /* ─ Viola ─ */
    "V_C3": "./music/all-samples/viola/viola_C3_phrase_mezzo-forte_arco-glissando.mp3",
    "V_E3": "./music/all-samples/viola/viola_E3_long_pianissimo_arco-sul-tasto.mp3",

    /* ─ Bassoon ─ */
    "B_C2": "./music/all-samples/bassoon/bassoon_C2_staccatissimo_forte.mp3",
    "B_D2": "./music/all-samples/bassoon/bassoon_D2_fall-gliss.mp3"
  };

  /* ===== ② 单一 Players。注意 disconnect()，自己接线 ===== */
  const fallPlayers = new Tone.Players(fallFiles, () =>
    console.log("🪂 fallPlayers ready")
  ).disconnect();          // 不直接到输出
...
function setup() {
  ...
  buildFallStereoBus();   // ★ 这里才创建 splitter / filter / autopan
                          //   并把 glissPlayers、fallExtraPlayers、bassPedShift
                          //   都接进去
